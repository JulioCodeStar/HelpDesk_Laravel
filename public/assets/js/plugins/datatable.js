...
// -----------------------------
// Reusable initializer — tableId required, extraOptions optional
// -----------------------------
window.initDT = (tableId, extraOptions = {}) => {
    const $table = $(`#${tableId}`);
    if (!$table.length) return;

    const { initComplete: userInitComplete, ...rest } = extraOptions;
    ...
    return $table.DataTable({
        ...
        ...rest,
        initComplete() {
            safeAddClass(`#${tableId}_wrapper > .row.mt-2.justify-content-between`, 'mx-2 py-2');
